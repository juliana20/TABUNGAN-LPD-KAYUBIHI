<form  method="POST" action="{{ url($submit_url) }}" class="form-horizontal" name="form_crud">
  ...
  <div class="form-group">
    ...
    <div class="col-lg-9">
      <input type="text" class="form-control" name="f[id_simpanan_pokok]" id="id_simpanan_pokok" value="{{ (!empty(@$item->id_simpanan_pokok)) ? @$item->id_simpanan_wajib : @$no_bukti }}" placeholder="No Bukti" required="" readonly>
    </div>
  </div>
  <div class="form-group">
  ...
  <div class="form-group">
    ...
    <div class="col-lg-9">
      ...
      <input type="text" class="form-control" name="f[no_rek_sim_pokok]" id="no_rek_sim_pokok" value="{{ @$item->no_rek_sim_pokok }}" placeholder="No Rekening" required="" readonly>
    </div>
  </div>
  <div class="form-group">
  ...
  <div class="form-group">
    <label class="col-lg-3 control-label">Penarikan</label>
    <div class="col-lg-9">
      <div class="input-group">
        <div class="input-group-btn">
          <span class="btn btn-default btn-flat">Rp</span>
        </div>
        <input type="text" class="form-control mask-number" name="f[debet]" id="penarikan" value="{{ @$item->debet }}" placeholder="Penarikan" autocomplete="off">
      </div>
    </div>
  </div>
  <div class="form-group">
...
$(document).on("keyup",'#penarikan', calculate);
  ...
  $('form[name="form_crud"]').on('submit',function(e) {
    e.preventDefault();
    penarikan = mask_number.currency_remove($('#penarikan').val());
    if(penarikan == '' || penarikan == 0)
    {
      $.alert_warning('Nominal penarikan belum di masukkan!');
      return false
    }
    ...
    var post_data = {
          'tanggal' : $("#tanggal").val(),
          'no_rek_sim_pokok' : $("#no_rek_sim_pokok").val(),
          'id_nasabah' : $("#id_nasabah").val(),
          'debet' : mask_number.currency_remove($("#penarikan").val()),
          'id_akun' : $("#id_akun").val(),
          'total_simp_pokok' : mask_number.currency_remove($("#saldo_akhir").val())
        }
