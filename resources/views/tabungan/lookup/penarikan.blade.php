<form  method="POST" action="{{ url($submit_url) }}" class="form-horizontal" name="form_crud">
  ...
  <div class="form-group">
    ...
    <div class="col-lg-9">
      <input type="text" class="form-control" name="f[id_tabungan_sukarela]" id="id_tabungan_sukarela" value="{{ (!empty(@$item->id_tabungan_sukarela)) ? @$item->id_tabungan_sukarela : @$no_bukti }}" placeholder="No Bukti" required="" readonly>
    </div>
  </div>
  <div class="form-group">
  ...
  <div class="form-group">
    ...
    <div class="col-lg-9">
      <div class="input-group">
        ...
        <input type="text" class="form-control mask-number" name="f[saldo]" id="saldo_akhir" value="{{ @$saldo }}" placeholder="Saldo Akhir" readonly>
      </div>
    </div>
  </div>
  <div class="form-group">
...
$(document).on("keyup",'#penarikan', calculate);
  ...
  $('form[name="form_crud"]').on('submit',function(e) {
    ...
    var post_data = {
          ...
          'id_akun' : $("#id_akun").val(),
          'saldo' : mask_number.currency_remove($("#saldo_akhir").val())
        }
    ...
    $.post($(this).attr("action"), data_post, function(response, status, xhr) {
      if( response.status == "error"){
          ...
          $.alert_success('Penarikan tabungan sukarela berhasil dibuat');
              $('.btn-save').removeClass('disabled');
              $(".spinner").css("display", "none");
              ajax_modal.hide();
              setTimeout(function(){
