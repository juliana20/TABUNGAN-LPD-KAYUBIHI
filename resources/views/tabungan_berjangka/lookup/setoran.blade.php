  ...
  <div class="form-group">
    ...
    <div class="col-lg-9">
      <div class="input-group">
        ...
        <input type="text" class="form-control mask-number" name="f[kredit]" id="setoran" value="{{ (!empty(@$item->kredit)) ? @$item->kredit : @$item->nominal_tabungan_berjangka }}" placeholder="Setoran" autocomplete="off" readonly>
      </div>
    </div>
  </div>
  <div class="form-group">
  ...
  $('form[name="form_crud"]').on('submit',function(e) {
    ...
    $.post($(this).attr("action"), data_post, function(response, status, xhr) {
      if( response.status == "error"){
          ...
          $.alert_success('Setoran tabungan berjangka berhasil!');
              $('.btn-save').removeClass('disabled');
              $(".spinner").css("display", "none");
              ajax_modal.hide();
              setTimeout(function(){
