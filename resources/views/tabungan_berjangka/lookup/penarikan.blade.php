<form  method="POST" action="{{ url($submit_url) }}" class="form-horizontal" name="form_crud">
  ...
  <div class="form-group">
      <div class="col-lg-offset-3 col-lg-9">
        ...
        <button type="submit" class="btn btn-success btn-save"><i class="fas fa-spinner fa-spin spinner" style="display: none"></i> Simpan</button>
      </div>
  </div>
</form>
...
<script type="text/javascript">
  ...
  $('form[name="form_crud"]').on('submit',function(e) {
    ...
    var post_data = {
          ...
          'id_akun' : $("#id_akun").val(),
          'saldo_akhir' :  mask_number.currency_remove($("#saldo").val()),
        }
    ...
    $.post($(this).attr("action"), data_post, function(response, status, xhr) {
      if( response.status == "error"){
          ...
          $.alert_success('Penarikan tabungan berjangka berhasil!');
              $('.btn-save').removeClass('disabled');
              $(".spinner").css("display", "none");
              ajax_modal.hide();
              setTimeout(function(){
